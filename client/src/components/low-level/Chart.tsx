...
// export default function SmoothPurpleGraph({ metrics,allLeveragePositions }: { metrics: Metrics[], allLeveragePositions:LeveragePosition[] }) {
...
export default function SmoothPurpleGraph({
  metrics,
  ...
  // ✅ Calculate cumulative amount deposited over time
  const cumulativeDeposits: number[] = [];
  let runningTotal = 0;

  allLeveragePositions.forEach((pos) => {
    runningTotal += Number(pos.amountDepositedInUsd) || 0;
    cumulativeDeposits.push(runningTotal);
  });

  // Prevent empty array case
  if (cumulativeDeposits.length === 0) cumulativeDeposits.push(0);

  // ✅ Normalize Y-values for chart scaling
  const minDeposit = Math.min(...cumulativeDeposits);
  const maxDeposit = Math.max(...cumulativeDeposits);
  const range = maxDeposit - minDeposit || 1;

  // ✅ Convert cumulative deposits into chart points
  const points = cumulativeDeposits.map((deposit, i) => ({
    x: (i / (cumulativeDeposits.length - 1)) * width,
    y: chartHeight - 8 - ((deposit - minDeposit) / range) * (chartHeight - 40),
  }));
  ...
  return (
    ...
    >
      ...
      <div className="flex justify-between items-center px-[20px] lg:px-[24px]">
        <div className="text-xs sm:text-sm text-gray-400 truncate">
          {selected !== null && allLeveragePositions[selected]
            ? `${new Date(
                Date.now() -
                  Number(allLeveragePositions[selected].openedOn) *
                    24 *
                    60 *
                    60 *
                    1000
        ...
        <div className="text-base sm:text-lg font-bold text-white">
          {selected !== null && cumulativeDeposits[selected] !== undefined
            ? `$${formatNumber(cumulativeDeposits[selected])}`
            : ""}
        </div>
      </div>
